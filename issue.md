...
### 第一梯队（播放器基本盘）
1. **播放模式** —— 目前仅顺序播放；缺：随机、单曲循环、列表循环
   （改动成本低：`_switch_song` 加模式分支，QML 播放模式按钮 + 状态保存）
2. **媒体键 + MPRIS 集成（Linux 最重要）** —— 键盘媒体键、桌面音量 OSD、
   蓝牙/锁屏控制条均依赖 MPRIS（D-Bus 接口）；目前完全缺失。
   可用 `mpris2`（python）+ QDBus 或直接 `python-mpris2` 库。
3. **播放队列** —— 右键"下一首播放"、搜索结果加入队列、拖拽排序、队列视图
