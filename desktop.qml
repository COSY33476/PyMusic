...
// 桌面歌词 QML。窗口由 desktop.py 的 QQuickView 提供（无边框/透明/置顶等
// 在 Python 侧设置），根元素用 Item（QQuickView 不允许 Window 作根对象）。
//
...
Item {
    ...
    //   取消锁定 → 左键 startSystemMove() 拖动整窗，右键弹菜单。
    // Wayland 下拖动后客户端拿不到新坐标（协议限制），onReleased 只通知
    // Python"拖动结束"，真实坐标由 Python 借道 KWin 脚本查询（见
    // desktop.py _KWinPinner.query_position）。
    MouseArea {
        id: dragArea
        anchors.fill: parent
        z: 10
